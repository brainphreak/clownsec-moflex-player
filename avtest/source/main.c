...
int main(void) {
    ...
    while (aptMainLoop()) {
        ...
        if (g_vqn >= 2 && (wr - rd) < NBUF - 1) {
            ...
            /* reached the keyframe -> stop skipping and decode it. Instead of mobi_flush (which nulls the
             * pool -> post-skip P-frames hit null data -> decode error -> freeze), we decode the keyframe
             * then seed the whole reference pool with it (below) -> references are valid keyframe content,
             * so no ghosting AND no freeze. */
